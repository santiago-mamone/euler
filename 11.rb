...
grid.each { |row| row.map! { |n| n.to_i } }
...
# vertical groups
0.upto(grid.size - 4) do |i|
  0.upto(column_count - 1) do |j|
    prod = grid[i][j]*grid[i+1][j]*grid[i+2][j]*grid[i+3][j]
    max = prod if prod > max
  end
end

# diagonal groups, from top-left to bottom-right
0.upto(grid.size - 4) do |i|
  0.upto(column_count - 4) do |j|
...
# diagonal groups, from top-right to bottom-left
0.upto(grid.size - 4) do |i|
  3.upto(column_count - 1) do |j|
    prod = grid[i][j]*grid[i+1][j-1]*grid[i+2][j-2]*grid[i+3][j-3]
    max = prod if prod > max
  end
end
